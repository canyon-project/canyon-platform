...
const ReportDetail: React.FC = ({visible,closeVisible,commitSha,dataSource,selectCommit}:any) => {
    const onClose = () => {
        closeVisible()
    };
    return (
        <>
            <Drawer title="上报日志" placement="right" width={'65%'} onClose={onClose}  visible={visible}>
                <Table rowKey={'id'} columns={columns} dataSource={dataSource.find((item:any)=>item.commitSha === selectCommit)?.record||[]}/>
            </Drawer>
        </>
    );
};
