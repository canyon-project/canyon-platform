...
const CoverageReport: FC<CoverageReportProps> = (props) => {
  ...
  const statusLineColor = calculateLv(
    percent(
      selectFile.statistics['statements'].covered,
      selectFile.statistics['statements'].total,
    ),
    watermarks,
  ).color
    ...
    <div className={'coverage-report'}>
      <div className={'tree-file-dir'}>
        <TreeFileDir
          ...
          controlSelectFile={controlSelectFile}
        />
      </div>
            ...
            {Object.keys(selectFile.statistics).map((item, index) => {
              return (
                <div style={{ marginRight: '20px' }} key={index}>
                  ...
                  >
                    {percent(
                      selectFile.statistics[item].covered,
                      selectFile.statistics[item].total,
                    )}
                    %
                  </span>
