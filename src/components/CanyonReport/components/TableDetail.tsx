  ...
  testMap.forEach((item) => {
    c.push({
      ...
      render(_: any) {
        ...
        return (
          <div
            style={{ color: calculateLv(per, watermarks).color }}
          >{`${per}%`}</div>
        )
      },
      colSpan: 2,
    ...
    c.push({
      ...
      render(_: any) {
        ...
        const per = percent(covered, total)
        return (
          <span style={{ color: calculateLv(per, watermarks).color }}>{`${
            covered + '/' + total
          }`}</span>
        )
      },
  ...
  const columns = [
    ...
    },
    {
      ...
      render(_: any) {
        ...
        return (
          <Progress
            strokeColor={calculateLv(per, watermarks).color}
            showInfo={false}
            percent={per}
          />
        )
  ...
  return (
    <div>
      ...
      <Table
        ...
        dataSource={props.tableSummary.children}
        columns={columns}
      />
    </div>
  )
