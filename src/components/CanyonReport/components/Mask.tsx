...
function init(content) {
  ...
  const maxIndex = rows1
    .map((item) => item.length)
    .findIndex((item) => item === max)
  // @ts-ignore
  const unitWidth =
    (document.getElementsByClassName(' CodeMirror-line')[maxIndex]?.children[0]
      .offsetWidth || 0) / max
  // @ts-ignore
  const unitHeight =
    document.getElementsByClassName('CodeMirror-line')[0].offsetHeight
  // @ts-ignore
  const positionLeft =
    document.getElementsByClassName('CodeMirror-linenumber')[0].offsetWidth + 4
  const positionTop = 4
...
const Mask = ({ data, edMaxWidth, rows }) => {
  ...
  // 初始化
  const { unitWidth, unitHeight, positionLeft, positionTop } = init(content)
  useEffect(() => {
