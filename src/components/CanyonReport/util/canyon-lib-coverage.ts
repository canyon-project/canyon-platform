...
export function mergeFileCoverageSummary(a: any, b: any): Summary {
  ...
  for (const mergeSummaryKey in mergeSummary) {
    mergeSummary[mergeSummaryKey].total =
      a[mergeSummaryKey].total + b[mergeSummaryKey].total
    mergeSummary[mergeSummaryKey].covered =
      a[mergeSummaryKey].covered + b[mergeSummaryKey].covered
    mergeSummary[mergeSummaryKey].skipped =
      a[mergeSummaryKey].skipped + b[mergeSummaryKey].skipped
  }
  return mergeSummary
}

export function genTree(cov: any) {
  const treeDTO = [{ key: '__covering__', children: [] }]
  // 删除第一个/
  ...
  cov.forEach((item: any) => {
    const key = item.path
    const nodeArray = key.split('/')
    let children = treeDTO
    // 构建根节点
    // 循环构建子节点
    for (let i = 0; i < nodeArray.length; i++) {
      // 获取全路径
      const fullPath = nodeArray
        .filter(
          (_: any, index: number) => index < Number(i) || index === Number(i),
        )
        .join('/')
      ...
      const node: any = {
        ...
        isLeaf: isFile,
        data: isFile
          ? fileCoverageSummary(cov.find((item: any) => item.path === fullPath))
          : {},
      }
...
export function genTreeSummary(jsonx: any) {
  const json = { children: JSON.parse(JSON.stringify(jsonx)) } || {
    children: [],
  }
  ...
  function getLeafCountTree(json: any) {
    if (json.children.length === 0) {
      ...
      for (let i = 0; i < json.children.length; i++) {
        leafCount = mergeFileCoverageSummary(
          leafCount,
          getLeafCountTree(json.children[i]),
        )
      }
